...
static void run_scenario_2(bert_ctx * ctx, const bench_params & p,
                         const char * label, int target_tokens, int batch_size) {
    std::string prompt = make_prompt(target_tokens);
    bert_tokens single_seq_toks = bert_tokenize(ctx, prompt, bert_n_max_tokens(ctx));
    
    // Ensure standard baseline size
    int target_len = target_tokens;
    if ((int)single_seq_toks.size() > target_len) {
        single_seq_toks.resize(target_len);
    } else {
        while ((int)single_seq_toks.size() < target_len) {
            single_seq_toks.push_back(0); 
        }
    }

    // ─────────────────────────────────────────────────────────────────
    // Alternate Strategy: Build a contiguous 1D token vector 
    // flattened across the entire targeted batch size.
    // ─────────────────────────────────────────────────────────────────
    bert_tokens flattened_tokens;
    flattened_tokens.reserve(target_len * batch_size);
    for (int i = 0; i < batch_size; ++i) {
        flattened_tokens.insert(flattened_tokens.end(), single_seq_toks.begin(), single_seq_toks.end());
    }

    // Wrap the flattened vector as a single-row "batch" containing the total token volume
    bert_batch batch;
    batch.push_back(flattened_tokens);
    
    int actual_tokens = target_len; 
...
static void run_scenario_1(bert_ctx * ctx, const bench_params & p,
                         const char * label, int target_tokens, int batch_size) {
    std::string prompt = make_prompt(target_tokens);
...
#ifdef OLD_CODE
#define run_scenario run_scenario_2
#else
#define run_scenario run_scenario_1
#endif 
